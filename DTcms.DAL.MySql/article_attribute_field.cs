...
namespace DTcms.DAL.MySql
{
    ...
    public partial class article_attribute_field
    {
            ...
            using (MySqlConnection conn = new MySqlConnection(DbHelperMySql.connectionString))
            {
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            DataTable dt = new DAL.MySql.site_channel(databaseprefix).GetFieldList(conn, trans, model.id).Tables[0];
                            if (dt.Rows.Count > 0)
                            {
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    int rowsCount = Convert.ToInt32(DbHelperMySql.GetSingle(conn, trans, "select count(1) from syscolumns where id=object_id('" + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + "') and name='" + oldModel.name + "'"));
                                    if (rowsCount > 0)
                                    {
                                        //修改列数据类型
                                        if (oldModel.data_type.ToLower() != model.data_type.ToLower())
                                        {
                                            DbHelperMySql.ExecuteSql(conn, trans, "alter table " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + " alter column " + oldModel.name + " " + model.data_type);
                                        }
                                        //修改列名
                                        if (oldModel.name.ToLower() != model.name.ToLower())
                                        {
                                            DbHelperMySql.ExecuteSql(conn, trans, "exec sp_rename '" + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + dr["name"].ToString() + "." + oldModel.name + "','" + model.name + "','column'");
                                        }
                                    }
                                }
                            }
        ...
        public bool Exists(string column_name)
        {
            ...
            if (DbHelperMySql.Exists(strSql.ToString(), parameters))
            {
                return true;
            }
            return false;
