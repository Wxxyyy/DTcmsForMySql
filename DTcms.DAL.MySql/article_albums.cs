...
namespace DTcms.DAL.MySql
{
    ...
    public partial class article_albums
    {
        ...
        /// 增加一条数据
        /// </summary>
        public void Add(MySqlConnection conn, MySqlTransaction trans, List<Model.article_albums> models, int channel_id, int article_id)
        {
            if (models != null)
            {
                ...
                foreach (Model.article_albums modelt in models)
                {
                            ...
                            if (pi.GetValue(modelt, null) != null)
                            {
                                ...
                                switch (pi.Name)
                                {
                                    case "channel_id":
                                        ...
                                        break;
                                    case "article_id":
                                        paras.Add(new MySqlParameter("@" + pi.Name, article_id));//刚插入的文章ID
                                        break;
                                    default:
                                        paras.Add(new MySqlParameter("@" + pi.Name, pi.GetValue(modelt, null)));//对参数赋值
                                        break;
                                }
                            }
                        }
                    ...
                    strSql.Append(") ");
                    DbHelperMySql.ExecuteSql(conn, trans, strSql.ToString(), paras.ToArray());//带事务
                }
            }
        }
        ...
        /// 更新一条数据
        /// </summary>
        public void Update(MySqlConnection conn, MySqlTransaction trans, List<Model.article_albums> models, int channel_id, int article_id)
        {
            //删除已移除的图片
            ...
            if (models != null)
            {
                ...
                foreach (Model.article_albums modelt in models)
                {
                    ...
                    if (modelt.id > 0)
                    {
                        ...
                        paras.Add(new MySqlParameter("@id", modelt.id));
        ...
        /// </summary>
        public void DeleteList(MySqlConnection conn, MySqlTransaction trans, List<Model.article_albums> models, int channel_id, int article_id)
        {
