            ...
            using (MySqlConnection conn = new MySqlConnection(DbHelperMySql.connectionString))
            {
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        new DAL.MySql.user_amount_log(databaseprefix).Add(conn, trans, amountModel);
                        #endregion
                        ...
                        strSql.Append(") ");
                        strSql.Append(";select @@IDENTITY;");
                        object obj = DbHelperMySql.GetSingle(conn, trans, strSql.ToString(), paras.ToArray());
            ...
            using (MySqlConnection conn = new MySqlConnection(DbHelperMySql.connectionString))
            {
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append(" where recharge_no=@recharge_no");
                        MySqlParameter[] parameters = {
					            new MySqlParameter("@status", MySqlDbType.TinyText,1),
					            new MySqlParameter("@complete_time", MySqlDbType.DateTime),
					            new MySqlParameter("@recharge_no", MySqlDbType.VarChar,100)};
